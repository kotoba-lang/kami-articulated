...
expressed with the sealed Kotoba XML ABI. It queries robot name, ordered link
names, ordered mass attributes, and ordered joint name, kind, parent, and child
attributes. It does not replace the full CLJC `ArticulatedSystem` constructor:
numeric parsing, optional pose/inertia fields, graph validation, and structured
...
2,048-node, depth-32, 32-attribute, and 32-path-segment limits. Absence is a
typed `[:option :string]`; malformed or excessive XML traps.
...
parser has been migrated. The next language gap is bounded numeric string
parsing plus typed record/graph assembly; the physics solver remains a separate
Kami engine provider boundary.

Evidence: kotoba-lang/compiler PR #136 merged as
`52726c74e1bd77a2a845a76d860666062c1f31b7`; this repository pins that exact
commit. `clojure -M:test` passes 8 tests and 38 assertions, including both real
